...
- [ ] Make the MVP list and the roadmap map one to one, as the template asks: today MVP items 1 and 2 both fall into C1, item 4 is split between C2 and C3, and C4 has no MVP item.
...
- [ ] Settle the four unresolved decisions in the roadmap (benchmark tolerance, expected benchmark values, who accepts results, failure inputs for steps with mixed LRV signs). Owners: Wolfgang and Malte.
