...
- [ ] With every failure frequency at 0, all results equal today's exactly, including the existing regression test's numbers.
- [ ] A step with failure frequency 365 gives the same worst-case results as the same scenario without that step's positive minimum LRVs.
- [ ] Raising a failure frequency never lowers any mean. Changing a failure duration leaves worst-case unchanged.
- [ ] Worst-case mean ≥ best-case mean for every reference pathogen and risk measure, with and without failures.
- [ ] D4: a failing step with a negative LRV in one pathogen group and a positive LRV in another leaves the first group's results unchanged. A group with LRV 0 is unaffected.
- [ ] Calculating the same scenario with failures twice gives identical results.
- [ ] All tests go through the calculation's public entry point and check only the returned results. The existing test suite passes unchanged.
