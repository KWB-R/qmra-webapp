...
The failure events of the day are arranged within 1,440 minutes so that the mixed concentration is lowest:
- if their durations fit into one day, they do not overlap;
- otherwise they overlap only as much as necessary, on the steps whose joint loss matters least.

The arrangement is chosen separately for each reference pathogen, because the LRVs differ per pathogen group. The failure days themselves stay shared. For a single failing step, the result equals Eq. 5 of the failure approach. Work per failure day is reused for repeated combinations of failing steps.
...
- [ ] Three steps always failing with more than a day in total equal the hand-computed lowest-concentration arrangement.
- [ ] Three steps whose least important pair differs between two pathogen groups: each reference pathogen equals its own hand-computed arrangement.
